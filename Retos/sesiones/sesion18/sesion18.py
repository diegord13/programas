...
def imprimirMatriz(matriz):
    print("Punto\tPlaca\tTiempo\tLitros")
    print("-----\t-----\t------\t------")
    for i in range(len(matriz)):
        for j in range(len(matriz[i])):
            print(matriz[i][j], end="\t")
        print() #Imprime salto de línea para imprimir nueva fila

def cargarMatrizAsignacion(nom_archivo):
    archivo = open(nom_archivo,"r")
    info_archivo = archivo.read()

    lista_lineas = info_archivo.split("\n")

    matriz = []
    for i in range(len(lista_lineas)):
        matriz.append(lista_lineas[i].split(","))
    
    return matriz

matriz_asignacion = cargarMatrizAsignacion("matrizAsignacion.txt")
imprimirMatriz(matriz_asignacion)
